    ...
    for attempt in range(30):
        try:
            _pg_conn = psycopg2.connect(
                ...
                user="postgres", password=PG_PASSWORD, dbname=PG_DB,
            )
...
def generate_response(message: str) -> str:
    ...
    return f"I received: '{message}'. Replace generate_response() with your LLM call."
...
async def chat(req: ChatRequest):
    ...
    with conn.cursor() as cur:
        cur.execute("INSERT INTO sessions (id) VALUES (%s) ON CONFLICT DO NOTHING", (session_id,))
        cur.execute("INSERT INTO messages (session_id, role, content) VALUES (%s, %s, %s)", (session_id, "user", req.message))
    ...
    with conn.cursor() as cur:
        cur.execute("INSERT INTO messages (session_id, role, content) VALUES (%s, %s, %s)", (session_id, "assistant", response))
        cur.execute("SELECT COUNT(*) FROM messages WHERE session_id = %s", (session_id,))
        count = cur.fetchone()[0]

    return ChatResponse(session_id=session_id, response=response, history_length=count)
